...
def execute_task(job: dict) -> dict:
    ...
    try:
        ...
        elif task_type == 'btc_price_alert':
            status, message = _btc_price_multi_channel(payload)
        elif task_type == 'software_jobs_alert':
            status, message = _software_jobs_alert(payload)
        else:
...
def _btc_price_multi_channel(payload: dict) -> Tuple[str, str]:
    """Send BTC price to all configured channels (Telegram, Discord, Email)."""
    text = payload.get('text', 'BTC Price Update')
    subject = payload.get('subject', 'BTC Price Alert')
    ...
    if DISCORD_WEBHOOK_URL:
        try:
            status, message = _send_discord({
                'content': text
            })
            results.append(('Discord', status))
            if status == 'failed':
                failed_channels.append(f'Discord: {message}')
        except Exception as e:
            failed_channels.append(f'Discord: {str(e)}')
    ...
    return ('success', f'BTC price sent to {len(success_channels)} channels: {", ".join(success_channels)}')
